...
def init_binance():
    ...
    logger.info(f'TRADING状态且标的资产为USDT的交易对数量：{len(symbols)}')
    for symbol in symbols:
        update_klines(symbol, '30m')


def update_klines(symbol_info, interval):
    symbol = symbol_info['symbol']
    create_table(symbol_info['symbol'])
    while True:
        with db.getconn() as conn:
            try:
                with conn.cursor() as cursor:
                    ...
                    logger.info(f'开始更新{symbol}的K线数据至：{local_time}')
                    # 更新数据库
                    for kline in klines:
    ...
    with db.getconn() as conn:
        try:
            with conn.cursor() as cursor:
                ...
                # 如果表不存在，则创建表
                if not table_exists:
                    logger.info(f'表{symbol}不存在，开始创建')
                    cursor.execute(f'''
                        CREATE TABLE "{symbol}" (
                    ...
                    logger.info(f'表{symbol}已存在，跳过创建')
                ...
                if not table_is_hypertable:
                    logger.info(f'表{symbol}不是超表，开始创建')
                    cursor.execute(f'''
                        SELECT create_hypertable('"{symbol}"', 'time');
                    ''')
                else:
                    logger.info(f'表{symbol}已是超表，跳过创建')

                # 提交数据库事务
                conn.commit()
        except Exception as e:
            logger.error(f'创建表{symbol}失败，原因：{e}')
            conn.rollback()
        finally:
            db.putconn(conn)
